...
PCAFigureCreation <- function(PCA,pc.perc,genoinfo,infilename, Colourant ){
  ...
  # genoinfo <- matching
  # Colourant <- "Program"
progfile <- paste("Figures/Plot_PCA_",infilename,"_by_",Colourant,".pdf",sep = "")
...
if(Colourant == "Program"){
  ...
  colors <- c("slategray3", "violet", "royalblue4", "palegreen", "mediumpurple",  "orange", "grey", "red")
  symbols <- c(20,20,15,20,20,20,20,17)
...
plot(tab$EV2, tab$EV1,pch = symbols[as.integer(tab$pop)], col = colors[as.integer(tab$pop)], xlab = paste("PC 2 (",(round(pc.perc,1))[2],"%)" ,sep = ""), ylab = paste("PC 1 (",(round(pc.perc,1))[1],"%)" ,sep = ""), cex.lab = 1.5)
